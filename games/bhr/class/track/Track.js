...
import { Lb, Mb, Hb } from "../../constant/var.js";
...
import Target from "../tool/item/Target.js";
import Checkpoint from "../tool/item/Checkpoint.js";
import Bomb from "../tool/item/Bomb.js";
import Boost from "../tool/item/Boost.js";
import Gravity from "../tool/item/Gravity.js";
import Antigravity from "../tool/item/Antigravity.js";
import Slowmo from "../tool/item/Slowmo.js";
import Teleporter from "../tool/item/Teleporter.js";
...
export default class {
    constructor(t) {
        ...
        this.vehicle = "BMX";
        this.players = [
            this.firstPlayer = new Player(this.vehicle, this)
        ];
        this.editor = 1;
    ...
    removeCollectedItems() {
        for (const t in this.grid) {
            if (this.grid.hasOwnProperty(t)) {
                for (const e in this.grid[t]) {
                    if (this.grid[t].hasOwnProperty(e)) {
                        for (const i of this.grid[t][e].powerups) {
                            if (i.used !== void 0) {
                                i.used = !1;
                            }
                        }
    ...
    watchGhost(a, b) {
        ...
        !function(a) {
            ...
            b.ghost_data = c;
            b.players.push(new Player(b.ghost_data[6], this, b.ghost_data));
            b.reset();
        });
        !function(a) {
            d = !0;
    ...
    draw() {
        let b = this.firstPlayer.vehicle
        , c = this.currentTime
        , d = this.zoom
        , e = this.scale
        ...
        if (this.cameraLock && !Hb && "line\\scenery line\\brush\\scenery brush\\teleporter".split(/\\/).includes(tool.selected))
            ...
            ctx.lineTo(f.x, f.y),
            ctx.stroke();
        let i = (new Vector(0,0)).toCanvas()
        , l = (new Vector(canvas.width,canvas.height)).toCanvas();
    ...
    undo() {
        if ("scenery line" === tool.selected || "scenery brush" === tool.selected) {
            ...
            let a = this.grid[Math.floor(Lb.x / this.scale)][Math.floor(Lb.y / this.scale)].physics[this.grid[Math.floor(Lb.x / this.scale)][Math.floor(Lb.y / this.scale)].physics.length - 1];
