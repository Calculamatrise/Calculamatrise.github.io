...
export default class MountainBike extends Vehicle {
    ...
    draw() {
        let a = this.track
        , b = this.rearWheel.pos.toPixel(this.track)
        , c = this.frontWheel.pos.toPixel(this.track)
        , d = this.head.pos.toPixel(this.track)
        , e = c.sub(b)
        , f = new Vector((c.y - b.y) * this.dir,(b.x - c.x) * this.dir)
        , h = d.sub(b.add(e.scale(0.5)));
        ctx.globalAlpha = this.ghost ? .5 : 1;
        ctx.strokeStyle = "#000";
        ctx.lineWidth = 3.5 * a.zoomFactor;
        ctx.beginPath(),ctx.arc(b.x, b.y, 12.5 * a.zoomFactor, 0, 2 * Math.PI, !0),ctx.moveTo(c.x + 12.5 * a.zoomFactor, c.y),ctx.arc(c.x, c.y, 12.5 * a.zoomFactor, 0, 2 * Math.PI, !0),ctx.stroke(),ctx.beginPath(),ctx.fillStyle = "grey";
        ctx.moveTo(b.x + 5 * a.zoomFactor, b.y),ctx.arc(b.x, b.y, 5 * a.zoomFactor, 0, 2 * Math.PI, !0),ctx.moveTo(c.x + 4 * a.zoomFactor, c.y),ctx.arc(c.x, c.y, 4 * a.zoomFactor, 0, 2 * Math.PI, !0),ctx.fill(),ctx.beginPath(),ctx.lineWidth = 5 * a.zoomFactor;
        ctx.moveTo(b.x, b.y),ctx.lineTo(b.x + 0.4 * e.x + 0.05 * f.x, b.y + 0.4 * e.y + 0.05 * f.y),ctx.moveTo(b.x + 0.72 * e.x + 0.64 * h.x, b.y + 0.72 * e.y + 0.64 * h.y),ctx.lineTo(b.x + 0.46 * e.x + 0.4 * h.x, b.y + 0.46 * e.y + 0.4 * h.y),ctx.lineTo(b.x + 0.4 * e.x + 0.05 * f.x, b.y + 0.4 * e.y + 0.05 * f.y),ctx.stroke(),ctx.beginPath(),ctx.lineWidth = 2 * a.zoomFactor;
        let i = new Vector(6 * Math.cos(this.pedalSpeed) * a.zoomFactor,6 * Math.sin(this.pedalSpeed) * a.zoomFactor);
        ctx.moveTo(b.x + 0.72 * e.x + 0.64 * h.x, b.y + 0.72 * e.y + 0.64 * h.y),ctx.lineTo(b.x + 0.43 * e.x + 0.05 * f.x, b.y + 0.43 * e.y + 0.05 * f.y),ctx.moveTo(b.x + 0.45 * e.x + 0.3 * h.x, b.y + 0.45 * e.y + 0.3 * h.y),ctx.lineTo(b.x + 0.3 * e.x + 0.4 * h.x, b.y + 0.3 * e.y + 0.4 * h.y),ctx.lineTo(b.x + 0.25 * e.x + 0.6 * h.x, b.y + 0.25 * e.y + 0.6 * h.y),ctx.moveTo(b.x + 0.17 * e.x + 0.6 * h.x, b.y + 0.17 * e.y + 0.6 * h.y),ctx.lineTo(b.x + 0.3 * e.x + 0.6 * h.x, b.y + 0.3 * e.y + 0.6 * h.y),ctx.moveTo(b.x + 0.43 * e.x + 0.05 * f.x + i.x, b.y + 0.43 * e.y + 0.05 * f.y + i.y),ctx.lineTo(b.x + 0.43 * e.x + 0.05 * f.x - i.x, b.y + 0.43 * e.y + 0.05 * f.y - i.y),ctx.stroke(),ctx.beginPath(),ctx.lineWidth = a.zoomFactor;
        ctx.moveTo(b.x + 0.46 * e.x + 0.4 * h.x, b.y + 0.46 * e.y + 0.4 * h.y),ctx.lineTo(b.x + 0.28 * e.x + 0.5 * h.x, b.y + 0.28 * e.y + 0.5 * h.y),ctx.stroke(),ctx.beginPath(),ctx.lineWidth = 3 * a.zoomFactor;
        ...
        if (!this.dead) {
            ...
            let m = d.sub(i);
            h = (new Vector(m.y * this.dir,-m.x * this.dir)).scaleSelf(a.zoomFactor * a.zoomFactor)
            let n = i.add(m.scale(0.5)).add(h.scale(200 / m.lengthSquared()));
            m = l.sub(i)
            , h = i.add(m.scale(0.5)).add(new Vector(m.y * this.dir,-m.x * this.dir).scaleSelf(a.zoomFactor * a.zoomFactor).scale(200 / m.lengthSquared()));
            ctx.beginPath(),ctx.lineWidth = 6 * a.zoomFactor;
            ctx.strokeStyle = "rgba(0, 0, 0, 0.5)";
            ctx.moveTo(l.x, l.y),ctx.lineTo(h.x, h.y),ctx.lineTo(i.x, i.y),ctx.stroke(),ctx.beginPath(),ctx.strokeStyle = "#000";
            ctx.moveTo(d.x, d.y),ctx.lineTo(n.x, n.y),ctx.lineTo(i.x, i.y),ctx.stroke(),ctx.lineWidth = 8 * a.zoomFactor;
            h = c.add(e.scale(0.1)).add(f.scale(0.93));
            d = c.add(e.scale(0.2)).add(f.scale(1.09));
            ctx.beginPath(),ctx.moveTo(i.x, i.y),ctx.lineTo(h.x, h.y),ctx.stroke(),ctx.beginPath(),ctx.lineWidth = 2 * a.zoomFactor;
            ctx.moveTo(d.x + 5 * a.zoomFactor, d.y),ctx.arc(d.x, d.y, 5 * a.zoomFactor, 0, 2 * Math.PI, !0),ctx.stroke(),ctx.beginPath();
            ...
            f = f.scale(a.zoomFactor * a.zoomFactor);
