        ...
        if (this.active) {
            ...
            temp.erase = function(event) {
                ...
                return !!points.find((point, index, points) => {
                    ...
                    let vector = {
                        x: (parseFloat(points[index - 1].x) - window.canvas.view.x) - (parseFloat(point.x) - window.canvas.view.x),
                        y: (parseFloat(points[index - 1].y) - window.canvas.view.y) - (parseFloat(point.y) - window.canvas.view.y)
                    }
                    let len = Math.sqrt(vector.x ** 2 + vector.y ** 2);
                    let b = (event.offsetX - (parseFloat(point.x) - window.canvas.view.x)) * (vector.x / len) + (event.offsetY - (parseFloat(point.y) - window.canvas.view.y)) * (vector.y / len);
                    const v = {
                        x: 0,
                        y: 0
                    }
                    ...
                    if (b <= 0) {
                        v.x = parseFloat(point.x) - window.canvas.view.x;
                        v.y = parseFloat(point.y) - window.canvas.view.y;
                    } else if (b >= len) {
                        v.x = parseFloat(points[index - 1].x) - window.canvas.view.x;
                        v.y = parseFloat(points[index - 1].y) - window.canvas.view.y;
                    } else {
                        v.x = (parseFloat(point.x) - window.canvas.view.x) + vector.x / len * b;
                        v.y = (parseFloat(point.y) - window.canvas.view.y) + vector.y / len * b;
                    }
