...
export default class extends Tool {
    ...
    get lines() {
        ...
        for (let i = 0; i <= 360; i += this.segmentLength) {
            ...
            temp.erase = function(event) {
                let vector = {
                    x: (parseInt(this.getAttribute("x2")) - window.canvas.view.x) - (parseInt(this.getAttribute("x1")) - window.canvas.view.x),
                    y: (parseInt(this.getAttribute("y2")) - window.canvas.view.y) - (parseInt(this.getAttribute("y1")) - window.canvas.view.y)
                }
                let len = Math.sqrt(vector.x ** 2 + vector.y ** 2);
                let b = (event.offsetX - (parseInt(this.getAttribute("x1")) - window.canvas.view.x)) * (vector.x / len) + (event.offsetY - (parseInt(this.getAttribute("y1")) - window.canvas.view.y)) * (vector.y / len);
                const v = {
                    x: 0,
                    y: 0
                }
                ...
                if (b <= 0) {
                    v.x = parseInt(this.getAttribute("x1")) - window.canvas.view.x;
                    v.y = parseInt(this.getAttribute("y1")) - window.canvas.view.y;
                } else if (b >= len) {
                    v.x = parseInt(this.getAttribute("x2")) - window.canvas.view.x;
                    v.y = parseInt(this.getAttribute("y2")) - window.canvas.view.y;
                } else {
                    v.x = (parseInt(this.getAttribute("x1")) - window.canvas.view.x) + vector.x / len * b;
                    v.y = (parseInt(this.getAttribute("y1")) - window.canvas.view.y) + vector.y / len * b;
                }
    ...
    mouseUp() {
        ...
        circle.erase = function(event) {
            let vector = {
                x: (parseInt(this.getAttribute("r")) - window.canvas.view.x) - (parseInt(this.getAttribute("cx")) - window.canvas.view.x),
                y: (parseInt(this.getAttribute("r")) - window.canvas.view.y) - (parseInt(this.getAttribute("cy")) - window.canvas.view.y)
            }
            let len = Math.sqrt(vector.x ** 2 + vector.y ** 2);
            let b = (event.offsetX - (parseInt(this.getAttribute("cx")) - window.canvas.view.x)) * (vector.x / len) + (event.offsetY - (parseInt(this.getAttribute("cy")) - window.canvas.view.y)) * (vector.y / len);
            const v = {
                x: 0,
                y: 0
            }
            ...
            if (b <= 0) {
                v.x = parseInt(this.getAttribute("cx")) - window.canvas.view.x;
                v.y = parseInt(this.getAttribute("cy")) - window.canvas.view.y;
            } else if (b >= len) {
                v.x = parseInt(this.getAttribute("r")) - window.canvas.view.x;
                v.y = parseInt(this.getAttribute("r")) - window.canvas.view.y;
            } else {
                v.x = (parseInt(this.getAttribute("cx")) - window.canvas.view.x) + vector.x / len * b;
                v.y = (parseInt(this.getAttribute("cy")) - window.canvas.view.y) + vector.y / len * b;
            }
